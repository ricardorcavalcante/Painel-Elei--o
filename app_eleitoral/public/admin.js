// admin.js — Central de Comando + aprovação de Agenda + gestão de equipe
// (destino do papel super_admin, Fase 2 da reestruturação por papel).
//
// Script clássico (globais), não ES module — mantém as funções migradas
// de public/app.js praticamente verbatim (mesmos nomes, mesma lógica),
// só trocando o "portão de entrada": em vez de refreshOKRSession() +
// checagem de aba dentro da SPA, quem decide se a página carrega é o
// guardPage() de auth-shared.js (chamado pelo <script type="module">
// no fim de admin.html), que já resolveu sessão + papel antes de
// initAdminPage(sb, ctx) ser chamado.
//
// Deliberadamente NÃO migrado pra cá (ver decisões da Fase 2):
// - gerarQuadrantesDaRA(): depende do subsistema de mapa (raLayer/
//   urbanLayer/ruralAreaLayer, google.maps.geometry) que só existe na
//   aba Mapa hoje — as 37 RAs já têm quadrante gerado via SQL, então o
//   botão "➕ Gerar" fica de fora do card de cada Coordenação aqui.
// - Painel de Artefatos: é evidência de Key Result, não gestão de
//   equipe — fica para okrs.html (Fase 2, próximo checkpoint).
// - Filtro por nível (Todos/Estratégico/Tático/Operacional): hoje vive
//   na barra lateral do OKRs, acoplado por acidente de layout ao
//   renderEquipe() da SPA — sem lista de OKRs nesta página pra
//   filtrar junto, não faz sentido replicar; Equipe aqui sempre mostra
//   tudo (currentOKRFilterLevel fica em 'all', o default de sempre).
...
// Stubs inertes: funções copiadas de app.js chamam estas variáveis de
// contexto de mapa, mas nesta página elas nunca são populadas — os
// próprios guards (`if (!map) return`) e o array vazio fazem o resto
// ser no-op, sem precisar reescrever as funções que as usam.
let map;
let areaRectangles = [];
...
// Memoizado (ao contrário do initSupabaseClient() de app.js, que cria
// um client novo a cada chamada): nesta página, várias funções deste
// arquivo disparam concorrentemente logo no cold-start do redirect
// pós-login (initAdminPage → Promise.allSettled), e criar um
// GoTrueClient por chamada faz eles brigarem pela mesma sessão no
// localStorage — na prática gera 401 esporádico. _sbInstance é
// semeada por initAdminPage() com o client que guardPage() (auth-
// shared.js) já criou, então todo o resto do arquivo reaproveita o
// mesmo client em vez de duplicar.
...
// Sem #okr-period-select/#okr-list-container nesta página — no-op
// seguro (mesmas funções de app.js, só os elementos-alvo não existem
// aqui, então saem no primeiro `if (!el) return`).
...
// ------------------------------------------
// Central de Comando (KPIs + 4 painéis)
// ------------------------------------------
...
// ------------------------------------------
// Bootstrap — chamado por login.html/admin.html depois que
// guardPage() (auth-shared.js) já confirmou papel super_admin.
// ------------------------------------------
async function initAdminPage(sb, ctx) {
    _sbInstance = sb;
    okrCurrentUser = ctx.profile;
